    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <button className="px-6 py-3 border-2 border-indigo-600 text-indigo-600 hover:bg-indigo-50 font-semibold rounded-lg transition-colors" onClick={() => navigate('/orders')}>
            ← Back to Orders
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-5xl mx-auto">
        <button className="mb-6 px-4 py-2 border-2 border-indigo-600 text-indigo-600 hover:bg-indigo-50 font-semibold rounded-lg transition-colors" onClick={() => navigate('/orders')}>
          ← Back to Orders
        </button>
                  ...
                  <div key={index} className="flex gap-4 p-4 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex-1">
                      ...
                      {item.service?.category?.name && (
                        <span className="inline-block px-2 py-1 bg-indigo-50 text-indigo-600 text-xs font-semibold rounded mb-2">{item.service.category.name}</span>
                      )}
          ...
          <div className="border-t border-gray-200 pt-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-xl font-bold text-gray-800 pt-3 border-t border-gray-200">
                <span>Total</span>
                <span className="text-indigo-600">৳{Math.round(parseFloat(order.total_price || subtotal))}</span>
              </div>
            </div>
          </div>
        </div>
